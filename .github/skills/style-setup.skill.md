...
spark: true
name: style-setup
description: 'Skill condivisa per la presentazione e selezione guidata dei parametri
  `verbosity` e `personality`. Definisce il formato di output per mostrare i valori
  correnti, le opzioni disponibili e il branching salta/personalizza. Usata da Agent-Welcome
  in OP-1 per la personalizzazione opzionale, e da Agent-Helper in modalità informativa
  read-only. La sezione di scrittura è autorizzata solo per Agent-Welcome durante
  OP-1 o OP-2.
...
1. Proponi la scelta con questo formato:
