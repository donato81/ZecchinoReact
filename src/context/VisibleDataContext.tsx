...
import {
  useVisibleData as useVisibleDataHook,
  type VisibleDataResult,
} from '@/hooks/use-visible-data';
...
export function useVisibleData(): VisibleDataContextValue {
  ...
  if (!ctx)
    throw new Error(
      'useVisibleData deve essere usato dentro VisibleDataProvider',
    );
  return ctx;
}
...
export function VisibleDataProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <VisibleDataContext.Provider value={data}>
      {children}
