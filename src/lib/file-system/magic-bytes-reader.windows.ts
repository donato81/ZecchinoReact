...
function decodeBase64(base64: string): Uint8Array {
  ...
  if (g.Buffer) {
    return new Uint8Array(g.Buffer.from(base64, 'base64')).slice(0, MAGIC_BYTES_READ_LENGTH)
  }

  const binary = atob(base64)
  return Uint8Array.from(binary, char => char.charCodeAt(0)).slice(0, MAGIC_BYTES_READ_LENGTH)
}
...
export async function readFileHeader(uri: string): Promise<Uint8Array> {
  try {
    ...
    const base64 = typeof fsModule.read === 'function'
      ? await fsModule.read(normalizeFileUri(uri), MAGIC_BYTES_READ_LENGTH, 0, 'base64')
