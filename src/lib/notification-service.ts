  ...
  return {
    ...
    async processBudgetNotifications(params: {
      ...
      for (const budget of activeBudgets) {
        ...
        const { shouldShow, level } = shouldShowBudgetNotification(
          budget,
          previousPercentage,
          percentage,
        );
        ...
        createdNotifications.push(
          await createNotification({
            tipo:
              budgetLevel === 'exceeded' ? 'budget_superato' : 'budget_soglia',
            titolo_key: `notifiche.budget.titolo.${budgetLevel}`,
            messaggio_key: `notifiche.budget.messaggio.${budgetLevel}`,
            entitaTipo: 'budget',
            entitaId: budget.id,
            livello: budgetLevel,
            metadata: {
