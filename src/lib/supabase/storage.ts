...
import {
  matchesSignature,
  readFileHeader,
} from '@/lib/file-system/magic-bytes-reader';
...
const EXTENSION_TO_SIGNATURE: Record<string, number[]> = {
  ...
  heic: [0, 0, 0, 0, 0x66, 0x74, 0x79, 0x70, 0, 0, 0, 0],
};
...
function sanitizeFilename(fileName: string): string {
  ...
  const normalizedPath =
    trimmed.replace(/\\/g, '/').split('/').pop()?.trim() ?? '';
...
function getExpectedMimeFromExtension(
  extension: string,
): AttachmentMimeType | null {
  return (
    (Object.entries(MIME_TO_EXTENSIONS).find(([, extensions]) =>
      extensions.includes(extension),
    )?.[0] as AttachmentMimeType | undefined) ?? null
  );
}
