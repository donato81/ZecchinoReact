...
function toDb(data: RecurrenceUpdateInput): Record<string, unknown> {
  ...
  if ('ultimaGenerazione' in data)
    out.ultima_generazione = data.ultimaGenerazione ?? null;
  if (data.prossimaGenerazione !== undefined)
    out.prossima_generazione = data.prossimaGenerazione;
...
export async function create(input: RecurrenceInput): Promise<Recurrence> {
  ...
  if (error)
    throw new RepositoryError(strings['errors.ricorrenze.createFailed']);
  return toClient(data as DbRecurrence);
}
...
export async function update(
  ...
  if (error)
    throw new RepositoryError(strings['errors.ricorrenze.updateFailed']);
  return toClient(data as DbRecurrence);
}
...
export async function deactivate(id: string): Promise<Recurrence> {
  ...
  if (error)
    throw new RepositoryError(strings['errors.ricorrenze.deactivateFailed']);
  return toClient(data as DbRecurrence);
}
