...
export async function getTagMapForTransactions(transactionIds: string[]): Promise<Record<string, string[]>> {
  ...
  const map = Object.fromEntries(transactionIds.map((transactionId) => [transactionId, [] as string[]]))
...
export async function setTagsForTransaction(transactionId: string, tagIds: string[]): Promise<void> {
...
export async function addTag(transactionId: string, tagId: string): Promise<void> {
  const { error } = await supabase.rpc('add_tag_to_transaction', {
    p_transaction_id: transactionId,
    p_tag_id: tagId,
  })

  if (error) throw new RepositoryError(strings['errors.tag.addToTransactionFailed'])
}

export async function removeTag(transactionId: string, tagId: string): Promise<void> {
  const { error } = await supabase.rpc('remove_tag_from_transaction', {
    p_transaction_id: transactionId,
    p_tag_id: tagId,
  })

  if (error) throw new RepositoryError(strings['errors.tag.removeFromTransactionFailed'])
}