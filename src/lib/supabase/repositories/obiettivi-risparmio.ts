...
function toDb(data: Partial<Omit<SavingsGoal, 'id'>>): Record<string, unknown> {
  ...
  if (data.importoCorrente !== undefined)
    out.importo_corrente = data.importoCorrente;
  ...
  if ('contoAssociato' in data)
    out.conto_associato = data.contoAssociato ?? null;
  ...
  if ('dataCompletamento' in data)
    out.data_completamento = data.dataCompletamento ?? null;
  return out;
}
...
export async function updateProgress(
  ...
  const { data, error } = await supabase.rpc('update_obiettivo_progress', {
    p_id: id,
