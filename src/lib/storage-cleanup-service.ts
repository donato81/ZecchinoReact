...
async function listCandidateFilesDefault(
  ...
  if (options.transazioneId) {
    const files = await listStoragePrefix(
      `${userId}/${options.transazioneId}`,
      options.limit,
    );
    return files
      .filter(isFileEntry)
      .map(file => ({
        path: `${userId}/${options.transazioneId}/${file.name}`,
  ...
  for (const entry of rootEntries) {
    ...
    const nestedFiles = await listStoragePrefix(
      `${userId}/${entry.name}`,
      remaining,
    );
    candidates.push(
      ...nestedFiles
        ...
        .slice(0, remaining)
        .map(file => ({
          path: `${userId}/${entry.name}/${file.name}`,
          createdAt: file.created_at,
        })),
    );
  }
...
async function listKnownPathsDefault(
  userId: string,
  pathPrefix?: string,
): Promise<Set<string>> {
  let query = supabase
  ...
  return new Set(
    ((data ?? []) as Array<{ storage_path: string }>).map(
...
function isOlderThanSafetyWindow(
...
function isRecentEnough(createdAt: string | undefined, nowMs: number): boolean {
  ...
  return (
    nowMs - new Date(createdAt).getTime() <=
    CLEANUP_RECENCY_HOURS * 60 * 60 * 1000
  );
}

export function createStorageCleanupService(
  customDeps?: Partial<StorageCleanupDeps>,
) {
  ...
  async function cleanupCandidates(
    ...
    if (!options.bypassGuards) {
      ...
      if (
        lastCleanupAt > 0 &&
        nowMs - lastCleanupAt < MIN_CLEANUP_INTERVAL_MS
      ) {
        return { ...EMPTY_RESULT };
      }
    ...
    try {
      const pathPrefix = options.transazioneId
        ? `${userId}/${options.transazioneId}/`
        : undefined;
      ...
      if (options.specificPath) {
        if (
          !options.specificPath.startsWith(userPrefix) ||
          knownPaths.has(options.specificPath)
        ) {
          return { ...EMPTY_RESULT };
        }
      ...
      for (const candidate of candidates) {
        ...
        if (
          options.applyRecencyFilter &&
          !isRecentEnough(candidate.createdAt, nowMs)
        ) {
          continue;
        }
        ...
        if (
          options.applySafetyWindow &&
          !isOlderThanSafetyWindow(candidate.createdAt, nowMs)
        ) {
          continue;
        }
