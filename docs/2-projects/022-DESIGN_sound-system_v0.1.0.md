...
> **Scope**: Riorganizzare il modulo audio dell'applicazione per supportare l'esecuzione in ambiente React Native nativo (Android, iOS). Sostituire le API Web Audio del browser (`window.AudioContext`, ecc.) con la libreria nativa `react-native-audio-api`. Ridurre drasticamente la superficie dei suoni supportati a 5 feedback canonici eliminando gli 87 suoni legacy inutilizzati. Stabilire la connessione mancante a runtime tra le impostazioni dell'utente (Supabase) e il Sound System.
>
...
Inoltre, il file definisce un'unione di tipi `SoundType` contenente **87 valori legacy** (verificati via analisi statica), la stragrande maggioranza dei quali non è agganciata ad alcuna schermata della UI reale, aggiungendo complessità inutile al codice e bloccando la standardizzazione dei feedback. 
...
### Decisione 3 — Riduzione a 5 suoni canonici e rimozione completa dei legacy
- **Descrizione**: Il tipo `SoundType` conterrà esattamente ed esclusivamente 5 suoni canonici:
  - `success`
  - `error`
  - `warning`
  - `click`
  - `navigation`
- **Motivazione**: Poiché la UI non è ancora implementata, non esistono consumatori reali da proteggere o mappare. Di conseguenza, tutti gli **87 suoni legacy** vengono eliminati immediatamente senza shims o tabelle di compatibilità. Suoni aggiuntivi verranno inseriti in modo incrementale solo quando esisteranno i relativi componenti grafici.
...
- **Frequenza**: $800\text{ Hz}$.
- **Durata**: $30\text{--}50\text{ ms}$.
- **ADSR**: Attacco istantaneo ($0.001\text{ s}$), decadimento rapido ($0.02\text{ s}$), sustain basso ($0.3$), rilascio rapido ($0.03\text{ s}$).
...
- **Forma d'onda**: `sine` (sinusoidale).
- **Pattern**: Sequenza ascendente a 3 note: $523.25\text{ Hz}$ (C5) $\to$ $659.25\text{ Hz}$ (E5) $\to$ $783.99\text{ Hz}$ (G5).
- **Durata**: $80\text{--}120\text{ ms}$ per nota (delay di interazione di circa $80\text{ ms}$).
...
- **Pattern**: Sequenza discendente a 2 note: $300\text{ Hz}$ $\to$ $250\text{ Hz}$.
- **Durata**: $100\text{--}150\text{ ms}$ per nota (delay di circa $100\text{ ms}$).
...
- **Forma d'onda**: `triangle` (triangolare) o `square` (onda quadra) a bassa intensità.
- **Pattern**: Doppio tono ripetuto: $440\text{ Hz}$ (A4) $\to$ $440\text{ Hz}$.
- **Durata**: $150\text{--}200\text{ ms}$ complessivi (delay di pausa intermedia di $150\text{ ms}$).
...
- **Forma d'onda**: `sine` (sinusoidale).
- **Frequenza**: $600\text{--}700\text{ Hz}$ (singolo tono leggero).
- **Durata**: $40\text{--}80\text{ ms}$.
- **ADSR**: Attacco morbido ($0.005\text{ s}$), decadimento rapido.
...
La nuova struttura del file [sound-system.ts](file:///c:/Sviluppo/ZecchinoReact/src/lib/sound-system.ts) dovrà implementare:
...
- **Importazione da libreria nativa**:
  ...
  ```
- **Union dei tipi semplificata**:
  ```typescript
  export type SoundType = 'click' | 'success' | 'error' | 'warning' | 'navigation';
  ```
- **Lazy Initialization**:
  ```typescript
...
La modifica del modulo audio introduce una nuova dipendenza nativa in [package.json](file:///c:/Sviluppo/ZecchinoReact/package.json):
...
- **T06** — `play('success')` sintetizza 3 note sine ad altezza ascendente ($523.25\text{ Hz} \to 659.25\text{ Hz} \to 783.99\text{ Hz}$).
- **T07** — `play('error')` genera 2 toni sawtooth discendenti ($300\text{ Hz} \to 250\text{ Hz}$).
- **T08** — `play('warning')` produce il pattern warning specificato.
- **T09** — `play('click')` produce un suono di click sine da $800\text{ Hz}$ e $30\text{--}50\text{ ms}$.
- **T10** — `play('navigation')` esegue il tono navigation specificato.
- **T11** — `ensureContext` implementa correttamente il caricamento lazy (costruttore vuoto, inizializzazione al primo play).
