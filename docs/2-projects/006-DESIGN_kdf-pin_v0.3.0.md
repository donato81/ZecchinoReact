...
**Libreria**: `@noble/hashes` (di Paul Miller), stessa famiglia di
`@noble/ciphers` già presente nel progetto. È una dipendenza separata
da `@noble/ciphers` e deve essere dichiarata esplicitamente in
`package.json`. La libreria è scritta in TypeScript puro, senza binding
nativi, ed è compatibile con il runtime Hermes su Android, iOS e Windows.
