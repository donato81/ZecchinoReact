...
BEGIN
  ...
  VALUES (p_transazione_id, p_tag_id)
  ON CONFLICT (transazione_id, tag_id) DO NOTHING;

  -- Incrementa il contatore utilizzi del tag
  UPDATE tag
  SET usato_n_volte = usato_n_volte + 1
  WHERE id = p_tag_id
    AND user_id = auth.uid()
    AND NOT EXISTS (
      SELECT 1 FROM transazioni_tag
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM unnest(p_tag_ids) AS t(id)
    WHERE NOT EXISTS (
      SELECT 1 FROM tag
      WHERE tag.id = t.id
  ...
  LOOP
    INSERT INTO transazioni_tag (transazione_id, tag_id)
    VALUES (p_transazione_id, v_tag_id)
    ON CONFLICT (transazione_id, tag_id) DO NOTHING;
  END LOOP;
