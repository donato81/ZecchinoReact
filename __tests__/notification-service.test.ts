...
const mockGetBudgetProgress = getBudgetProgress as jest.MockedFunction<
  typeof getBudgetProgress
>;
const mockShouldShowBudgetNotification =
  shouldShowBudgetNotification as jest.MockedFunction<
    typeof shouldShowBudgetNotification
  >;
const mockCreateNotification = createNotification as jest.MockedFunction<
  typeof createNotification
>;
const mockExistsUnreadForEntityLevel =
  existsUnreadForEntityLevel as jest.MockedFunction<
    typeof existsUnreadForEntityLevel
  >;
const mockGetUnreadByEntity = getUnreadByEntity as jest.MockedFunction<
  typeof getUnreadByEntity
>;
...
beforeEach(() => {
  jest.clearAllMocks();
  mockGetBudgetProgress.mockReturnValue({
    spent: 75,
    percentage: 75,
    remaining: 25,
    isOverBudget: false,
...
describe('notification-service', () => {
  it('deriva budgetPeriodKey dal riferimento locale della transazione e non dal mese UTC corrente', async () => {
    ...
    expect(mockExistsUnreadForEntityLevel).toHaveBeenCalledWith(
      expect.objectContaining({ budgetPeriodKey: '2026-04-01:2026-06-30' }),
    );
    expect(mockCreateNotification).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          budgetPeriodKey: '2026-04-01:2026-06-30',
        }),
      }),
    );
  });
  ...
  it('marca come lette le notifiche precedenti dello stesso periodo prima di creare una escalation', async () => {
    ...
    mockGetUnreadByEntity.mockResolvedValue([
      { id: 'notif-old', letta: false } as never,
    ]);
