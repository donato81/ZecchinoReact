...
describe('tag.repository', () => {
  ...
  it('getById restituisce il tag richiesto quando presente', async () => {
    ...
    await expect(getById('tag-1')).resolves.toEqual(
      expect.objectContaining({ id: 'tag-1', nome: 'Casa' }),
    );
  ...
  it('update modifica parzialmente un tag esistente senza alterare i campi non presenti', async () => {
    const chain = buildUpdateChain({
      data: { ...TAG_ROW, nome: 'Casa aggiornata' },
      error: null,
    });
  ...
  it('il repository tag propaga gli errori di lettura o scrittura con handling coerente', async () => {
    ...
    buildInsertChain({
      data: null,
      error: { message: 'insert', code: '500', details: '', hint: '' },
    });
    await expect(create({ nome: 'Casa' })).rejects.toThrow(
      'Impossibile creare il tag.',
    );

    buildUpdateChain({
      data: null,
      error: { message: 'update', code: '500', details: '', hint: '' },
    });
    await expect(update('tag-1', { nome: 'Errore' })).rejects.toThrow(
      'Impossibile aggiornare il tag.',
    );

    buildDeleteChain({
      error: { message: 'delete', code: '500', details: '', hint: '' },
    });
    await expect(remove('tag-1')).rejects.toThrow(
