...
function makeService(now: string) {
  const deps = {
    listCandidateFiles: jest.fn<Promise<Array<{ path: string; createdAt?: string }>>, [string, { transazioneId?: string; limit: number }]>(),
    listKnownPaths: jest.fn<Promise<Set<string>>, [string, string | undefined]>(),
...
describe('storage-cleanup-service', () => {
  it('Trigger 1 elimina il file orfano corretto e nessun altro', async () => {
    ...
    await expect(service.cleanupSpecificOrphan('user-016', 'user-016/tx-1/orphan.pdf')).resolves.toEqual({
      scanned: 1,
      orphanFound: 1,
      deleted: 1,
      failed: 0,
  ...
  it('Trigger 2 al login scansiona solo ultime 48 ore e al massimo MAX_FILES_PER_SCAN file', async () => {
    ...
    deps.listCandidateFiles.mockResolvedValue([
      { path: 'user-016/tx-1/recent.pdf', createdAt: '2026-05-28T02:00:00.000Z' },
    ...
    expect(deps.listCandidateFiles).toHaveBeenCalledWith('user-016', { limit: MAX_FILES_PER_SCAN, transazioneId: undefined })
  })
  ...
  it('Trigger 3 dopo cancellazione transazione limita la scansione al path user_id/transazione_id', async () => {
    ...
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockResolvedValue([
      { path: 'user-016/tx-42/orphan.pdf', createdAt: '2026-05-28T06:00:00.000Z' },
    ])
    deps.deleteFile.mockResolvedValue(undefined)

    await service.cleanupTransactionOrphans('user-016', 'tx-42')

    expect(deps.listCandidateFiles).toHaveBeenCalledWith('user-016', { limit: MAX_FILES_PER_SCAN, transazioneId: 'tx-42' })
    expect(deps.listKnownPaths).toHaveBeenCalledWith('user-016', 'user-016/tx-42/')
  })
  ...
  it('Trigger 4 al logout rispetta CLEANUP_LOGOUT_TIMEOUT_MS e non blocca il logout', async () => {
    ...
    const { service, deps } = makeService('2026-05-28T12:00:00.000Z')
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockImplementation(() => new Promise(() => undefined))

    const cleanupPromise = service.cleanupOnLogout('user-016')
    jest.advanceTimersByTime(CLEANUP_LOGOUT_TIMEOUT_MS)

    await expect(cleanupPromise).resolves.toEqual({ scanned: 0, orphanFound: 0, deleted: 0, failed: 0 })
    jest.useRealTimers()
  })

  it('cleanupInProgress blocca il secondo avvio concorrente, escluso Trigger 1', async () => {
    const { service, deps } = makeService('2026-05-28T12:00:00.000Z')
    let resolveList!: (value: Array<{ path: string; createdAt?: string }>) => void
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockImplementation(() => new Promise(resolve => {
      resolveList = resolve
    }))
    deps.deleteFile.mockResolvedValue(undefined)
    ...
    resolveList([{ path: 'user-016/tx-1/orphan.pdf', createdAt: '2026-05-28T06:00:00.000Z' }])

    await expect(second).resolves.toEqual({ scanned: 0, orphanFound: 0, deleted: 0, failed: 0 })
    await expect(first).resolves.toEqual({ scanned: 1, orphanFound: 1, deleted: 1, failed: 0 })
  })

  it('il throttle temporale impedisce un cleanup entro MIN_CLEANUP_INTERVAL_MS, escluso Trigger 1', async () => {
    const { service, deps } = makeService('2026-05-28T12:00:00.000Z')
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockResolvedValue([{ path: 'user-016/tx-1/orphan.pdf', createdAt: '2026-05-28T06:00:00.000Z' }])
    deps.deleteFile.mockResolvedValue(undefined)

    await service.cleanupRecentOrphans('user-016')
    await expect(service.cleanupRecentOrphans('user-016')).resolves.toEqual({ scanned: 0, orphanFound: 0, deleted: 0, failed: 0 })
    await expect(service.cleanupSpecificOrphan('user-016', 'user-016/tx-1/manual.pdf')).resolves.toEqual({
      scanned: 1,
      orphanFound: 1,
      deleted: 1,
      failed: 0,
  ...
  it('i file piu recenti di CLEANUP_SAFETY_WINDOW_MS non vengono eliminati', async () => {
    ...
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockResolvedValue([
      { path: 'user-016/tx-1/fresh.pdf', createdAt: '2026-05-28T11:58:30.000Z' },
    ])
  ...
  it("il cleanup non tocca file con path fuori dal prefisso user_id dell'utente", async () => {
    ...
    deps.listKnownPaths.mockResolvedValue(new Set())
    deps.listCandidateFiles.mockResolvedValue([
      { path: 'other-user/tx-1/orphan.pdf', createdAt: '2026-05-28T06:00:00.000Z' },
    ])
  ...
  it('CleanupResult riflette esattamente scanned, orphanFound, deleted e failed', async () => {
    ...
    deps.listKnownPaths.mockResolvedValue(new Set(['user-016/tx-1/known.pdf']))
    deps.listCandidateFiles.mockResolvedValue([
      { path: 'user-016/tx-1/deleted.pdf', createdAt: '2026-05-28T06:00:00.000Z' },
      { path: 'user-016/tx-1/fail.pdf', createdAt: '2026-05-28T06:00:00.000Z' },
      { path: 'user-016/tx-1/known.pdf', createdAt: '2026-05-28T06:00:00.000Z' },
    ])
    deps.deleteFile.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error('boom'))
  ...
  it('un errore su un singolo file non blocca l elaborazione degli altri orfani', async () => {
    ...
    deps.deleteFile.mockRejectedValueOnce(new Error('boom')).mockResolvedValueOnce(undefined)
