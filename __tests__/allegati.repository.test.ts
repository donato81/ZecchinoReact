...
describe('allegati.repository', () => {
  it('rollback upload con Storage OK, DB FAIL e tentativo di delete su Storage', async () => {
    const chain = buildInsertChain({ data: null, error: { message: 'insert fail', code: '500', details: '', hint: '' } })
    ...
    expect(mockUploadAttachment).toHaveBeenCalledWith('user-016', 'tx-016', expect.objectContaining({ name: 'documento.pdf' }))
  ...
  it('ordine cancellazione con Storage FAIL e DB non toccato', async () => {
    ...
    const eq: LookupChain['eq'] = jest.fn((_: string, __: string) => selectChain)
    selectChain = {
      eq,
  ...
  it("isolamento utenti, utente A non puo accedere ai file dell'utente B", async () => {
    const chain = buildSelectChain({ data: null, error: { message: 'row not found', code: 'PGRST116', details: '', hint: '' } })
  ...
  it('getAll richiede il filtro obbligatorio per transazione e ordina gli allegati più recenti', async () => {
    ...
    await expect(getAll('tx-016')).resolves.toEqual([
      expect.objectContaining({ id: 'all-1', transazioneId: 'tx-016', nomeFile: 'documento.pdf' }),
    ])
