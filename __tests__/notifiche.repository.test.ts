...
const ROW = {
  ...
  user_id: 'user-015',
  tipo: 'budget_soglia',
  titolo: 'Attenzione budget',
  messaggio: 'Il budget Casa ha raggiunto il 75 percento.',
  letta: false,
  canale: 'inapp',
  schedulata_per: null,
  entita_tipo: 'budget',
...
function buildSelectChain(result: unknown) {
  ...
  return { select, eq: chain.eq, contains: chain.contains, order: chain.order, single: chain.single }
}
...
describe('notifiche.repository', () => {
  it('getAll restituisce le notifiche disponibili ordinate secondo il contratto del repository', async () => {
    ...
    await expect(getAll()).resolves.toEqual([
      expect.objectContaining({ id: 'notif-1', tipo: 'budget_soglia', createdAt: ROW.created_at }),
    ])
  ...
  it('getUnreadByEntity restituisce solo notifiche non lette per entity e period key richiesti', async () => {
    ...
    await expect(getUnreadByEntity({ entitaTipo: 'budget', entitaId: 'budget-1', budgetPeriodKey: '2026-05' })).resolves.toHaveLength(1)
    expect(chain.eq).toHaveBeenCalledWith('letta', false)
    expect(chain.eq).toHaveBeenCalledWith('entita_tipo', 'budget')
    expect(chain.eq).toHaveBeenCalledWith('entita_id', 'budget-1')
    expect(chain.contains).toHaveBeenCalledWith('metadata', { budgetPeriodKey: '2026-05' })
  })
  ...
  it('existsUnreadForEntityLevel distingue correttamente presenza e assenza di duplicati non letti', async () => {
    ...
    await expect(existsUnreadForEntityLevel({ entitaTipo: 'budget', entitaId: 'budget-1', level: 'warning', budgetPeriodKey: '2026-05' })).resolves.toBe(true)
    expect(chain.contains).toHaveBeenNthCalledWith(1, 'metadata', { level: 'warning' })
    expect(chain.contains).toHaveBeenNthCalledWith(2, 'metadata', { budgetPeriodKey: '2026-05' })
  })

  it('markAsRead marca una singola notifica come letta', async () => {
    const chain = buildUpdateChain({ data: { ...ROW, letta: true }, error: null })

    await expect(markAsRead('notif-1')).resolves.toEqual(expect.objectContaining({ letta: true }))
  ...
  it('markAllAsRead marca come lette tutte le notifiche pertinenti', async () => {
    ...
    await expect(markAllAsRead({ entitaTipo: 'budget', entitaId: 'budget-1' })).resolves.toBeUndefined()
    expect(chain.updateFn).toHaveBeenCalledWith({ letta: true })
    expect(chain.eq).toHaveBeenCalledWith('letta', false)
    expect(chain.eq).toHaveBeenCalledWith('entita_tipo', 'budget')
    expect(chain.eq).toHaveBeenCalledWith('entita_id', 'budget-1')
  ...
  it('removeExpired elimina le notifiche scadute solo quando richiesto dal lifecycle', async () => {
    ...
    await expect(removeExpired('2026-05-28T12:00:00.000Z')).resolves.toBeUndefined()
    expect(chain.lt).toHaveBeenCalledWith('schedulata_per', '2026-05-28T12:00:00.000Z')
  })

  it('cleanupReadExpiredBefore elimina o pulisce notifiche lette antecedenti alla soglia', async () => {
    const chain = buildDeleteChain({ error: null })

    await expect(cleanupReadExpiredBefore('2026-05-01T00:00:00.000Z')).resolves.toBeUndefined()
